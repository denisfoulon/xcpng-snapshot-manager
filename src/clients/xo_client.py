...
class XOClient:
    ...
    def scan_storage_repository(self, sr_uuid: str):
        ...
        return self._client.post(
            f"/srs/{sr_uuid}/actions/scan",
            headers=self._basic_auth_header(),
        )

    def wait_for_task(self, task_reference, timeout_seconds: int) -> dict:
        """Wait for an asynchronous XO task when the action returns one."""

        import time

        task_id = self._reference_uuid(task_reference)
        if not task_id:
            return task_reference if isinstance(task_reference, dict) else {}
        deadline = time.monotonic() + timeout_seconds
        terminal = {"success", "failure", "error", "canceled", "cancelled", "completed", "done"}
        while time.monotonic() < deadline:
            response = self._client.get(
                f"/tasks/{task_id}?wait=true",
                headers=self._basic_auth_header(),
            )
            ...
            if status in terminal:
                return response
            time.sleep(2)
        raise TimeoutError(f"Timed out waiting for XO task {task_id}.")

    @staticmethod
    def _reference_uuid(value) -> str:
        if isinstance(value, dict):
            value = value.get("id", value.get("href", value.get("url", "")))
